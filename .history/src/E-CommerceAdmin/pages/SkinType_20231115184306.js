...
const SkinType = () => {
  ...
  const fetchHandler = async () => {
    try {
      const { data } = await axios.get(`${Baseurl}api/v1/area`);
      setData(data);
    } catch (e) {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postHandler = async (e) => {
      e.preventDefault();
      try {
        const { data } = await axios.post(`${Baseurl}api/v1/area`, payload, Auth);
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `https://shahina-backend.vercel.app/api/v1/admin/SkinType/updateSkinType/${id}`,
          payload,
          Auth
        );
        toast.success(data.message);
  ...
  const handleDelete = async (ide) => {
    const url = `https://shahina-backend.vercel.app/api/v1/admin/SkinType/deleteSkinType/${ide}`;
    try {
