...
const Faq = () => {
  ...
  const deleteHandler = async (ide) => {
    try {
      const { data } = await axios.delete(`${Baseurl}api/v1/faq/${ide}`, Auth);
      toast.success(data.message);
      fetchData();
    } catch (e) {
    ...
    <>
      ...
      <section className="sectionCont">
        ...
        {data?.length === 0 ? (
          <NoData
            message={
              "Apologies, but it seems we don't have FAQs available at the moment"
            }
          />
        ) : (
          <>
